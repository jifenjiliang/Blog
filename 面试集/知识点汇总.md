...
<pre>
node的优点：
1.I/O密集型处理是node的强项，因为node的I/O请求都是异步的（如：sql查询请求、文件流操作操作请求、http请求...）
2.因为nodejs的I/O操作，底层是开启了多线程的,当同时有多个IO请求时，主线程会创建多个eio线程，以提高IO请求的处理速度　　　　
3.虽然nodejs的I/O操作开启了多线程，但是所有线程都是基于主线程开启的只能跑在一个进程当中还是不能充分利用cpu资源,利用pm2进程管理器可以解决这个问题,pm2 是一个带有负载均衡功能的Node应用的进程管理器.
node的缺点：
1.不擅长cpu密集型的操作,因为nodejs是单线程的
</pre>
